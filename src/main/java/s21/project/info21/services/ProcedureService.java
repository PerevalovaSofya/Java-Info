...
public class ProcedureService {
    ...
    public void saveData(Map<String, Object> result) throws IOException {

        file = new File("output_data.csv");
        absolutePath = file.getAbsolutePath();

        try (FileOutputStream fileOutputStream = new FileOutputStream(absolutePath, false)) {
            for (var a : result.entrySet()) {
                fileOutputStream.write(a.getKey().getBytes());
                fileOutputStream.write("   ".getBytes());
                fileOutputStream.write(a.getValue().toString().getBytes());
